...
verificaExp :: (String -> Bool) -> Exp -> Bool
verificaExp vars (Var var)   = vars var
...
verificaCmd :: (String -> Bool) -> Cmd -> ((String -> Bool), Bool)
verificaCmd vars (Dcl var)     = ((\x -> if x == var then True else vars x), True)
verificaCmd vars (Atr var exp) = (vars,     (vars var) && (verificaExp vars exp))
